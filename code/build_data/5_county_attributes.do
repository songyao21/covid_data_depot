...
capture cd "$data"
insheet using "./co-est2019-alldata.csv", comma clear
...
save ./temp/county_attributes, replace

******* land area data to compute population density
* change to data directory
capture cd "$data"
insheet using "./cbg_geographic_data.csv", comma clear
...
merge 1:1 state county using ./temp/county_attributes, keep(3) nogen
...
* change to data directory
capture cd "$data"
import delimited "./Cleaned County Commuter Patterns 5 years 2018.csv", varnames(2) clear
* change to git directory
capture cd "$repo"
...
merge 1:1 fips using ./temp/county_attributes, keep(2 3) nogen
...
use ./temp/county_demo, clear
merge 1:1 state county using ./temp/county_attributes, keep(2 3) nogen
outsheet using ./county_attributes.csv, comma replace
